...
/**
 * Project detail modals + carousels (FSKPM, Trombol) — static index.html.
 */
(function initProjectDetailModals() {
  const FSKPM_SLIDES = [8, 9, 10, 11, 12, 13].map(
    (n) => `public/files(fskpm)/${n}.jpg`
  );
  const TROMBOL_SLIDES = [19, 20, 21, 22].map(
    (n) => `public/files2(trombol)/${n}.jpg`
  );
   ...
   * @param {{ viewport: string, prev: string, next: string, dot: string, curr: string, total: string }} sel
   */
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (modalFskpm && modalFskpm.classList.contains('is-open')) closeModal(modalFskpm);
    if (modalTrombol && modalTrombol.classList.contains('is-open')) closeModal(modalTrombol);
  });
})();
